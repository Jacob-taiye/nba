...
$activePage = 'tools';
$stmt = $pdo->prepare('SELECT * FROM tools WHERE is_active=1 ORDER BY created_at DESC');
$stmt->execute();
$tools = $stmt->fetchAll();
?>
<!DOCTYPE html>
...
<head>
  ...
  <title>Updates & Tools — VirtualHub Pro</title>
  ...
  <div class="dash-main">
    <div class="topbar">
      <div class="topbar-left">
        ...
        <div class="page-title"><i class="fas fa-screwdriver-wrench" style="color:#0984e3"></i> Updates & Tools</div>
      </div>
    ...
    <div class="dash-content">
      <div class="section-card" style="margin-bottom:22px">
        <div class="section-card-body" style="padding:14px 20px">
          <span style="font-size:0.88rem;color:#3a5068"><i class="fas fa-circle-info" style="color:#1a8cff;margin-right:7px"></i>Buy any tool below — the download link appears instantly after purchase.</span>
        </div>
      </div>
      <div class="items-grid">
        <?php if (empty($tools)): ?>
          <div class="empty-state"><i class="fas fa-screwdriver-wrench"></i><p>No tools available yet.</p></div>
        <?php else: foreach ($tools as $f): ?>
          <div class="item-card">
            <div class="item-icon" style="background:#e8f9ff;color:#0984e3"><i class="fas fa-screwdriver-wrench"></i></div>
            ...
            <div class="item-footer">
              ...
              <button class="buy-btn" onclick="buyItem('tool',<?= $f['id'] ?>,this)"><i class="fas fa-bolt"></i> Buy Now</button>
            </div>
          </div>
