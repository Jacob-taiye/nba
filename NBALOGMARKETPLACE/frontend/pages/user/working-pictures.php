...
$activePage = 'working-pictures';
$stmt = $pdo->prepare('SELECT id,title,sample_image,price FROM working_pictures WHERE is_active=1 ORDER BY created_at DESC');
$stmt->execute();
...
<head>
  ...
  <title>Working Pictures — NBALOGMARKETPLACE</title>
  <link rel="stylesheet" href="../../css/style.css"/>
  <link rel="stylesheet" href="../../css/dashboard.css"/>
  <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.5.0/css/all.min.css"/>
  <style>
    .pic-grid { display:grid; grid-template-columns:repeat(auto-fill,minmax(240px,1fr)); gap:20px; }
    .pic-card { background:white; border:1.5px solid #d6ecff; border-radius:16px; overflow:hidden; transition:all 0.25s; }
    .pic-card:hover { box-shadow:0 8px 28px rgba(26,140,255,0.14); transform:translateY(-3px); border-color:#1a8cff; }
    .pic-thumb { width:100%; height:180px; object-fit:cover; display:block; background:#f0f7ff; }
    .pic-thumb-placeholder { width:100%; height:180px; background:linear-gradient(135deg,#e8f4ff,#d6ecff); display:flex; align-items:center; justify-content:center; font-size:2.5rem; color:#b3d9ff; }
    .pic-body { padding:16px; }
    .pic-title { font-family:'Outfit',sans-serif; font-weight:700; font-size:0.95rem; color:#0d1b2a; margin-bottom:12px; }
    .pic-footer { display:flex; align-items:center; justify-content:space-between; }
    .item-price { font-family:'Outfit',sans-serif; font-size:1.1rem; font-weight:800; color:#1a8cff; }
    .buy-btn { padding:9px 18px; background:linear-gradient(135deg,#8e44ad,#6c3483); color:white; border:none; border-radius:9px; font-family:'Outfit',sans-serif; font-weight:700; font-size:0.85rem; cursor:pointer; transition:all 0.2s; display:flex; align-items:center; gap:7px; }
    .buy-btn:hover { transform:translateY(-1px); box-shadow:0 4px 14px rgba(142,68,173,0.35); }
    ...
    .modal-close:hover { background:#f3e8ff; color:#8e44ad; }
    ...
    .copy-btn { padding:0 18px; background:#8e44ad; color:white; border:none; cursor:pointer; font-weight:700; font-size:0.85rem; transition:background 0.2s; white-space:nowrap; }
    .copy-btn:hover { background:#6c3483; }
    ...
    <div class="dash-content">
      <div class="section-card" style="margin-bottom:22px">
        <div class="section-card-body" style="padding:14px 20px">
          <span style="font-size:0.88rem;color:#3a5068"><i class="fas fa-circle-info" style="color:#8e44ad;margin-right:7px"></i>Browse picture previews below. After purchase, you'll receive the full download link instantly.</span>
        </div>
      </div>
      <div class="pic-grid">
        <?php if (empty($pictures)): ?>
          <div class="empty-state"><i class="fas fa-images"></i><p>No pictures available yet.</p></div>
        <?php else: foreach ($pictures as $p): ?>
          <div class="pic-card">
            <?php if (!empty($p['sample_image'])): ?>
              <img src="<?= htmlspecialchars($p['sample_image']) ?>" alt="<?= htmlspecialchars($p['title']) ?>" class="pic-thumb" loading="lazy"/>
            <?php else: ?>
              <div class="pic-thumb-placeholder"><i class="fas fa-image"></i></div>
            <?php endif; ?>
            <div class="pic-body">
              <div class="pic-title"><?= htmlspecialchars($p['title']) ?></div>
              <div class="pic-footer">
                <div class="item-price"><?= formatMoney((float)$p['price']) ?></div>
                <button class="buy-btn" onclick="buyItem('working_picture',<?= $p['id'] ?>,this)">
...
<div class="modal-overlay" id="purchaseModal">
  <div class="modal-box">
    <div class="modal-head">
      <div class="modal-title" id="modalTitle">🖼️ Your Picture Link</div>
      <button class="modal-close" onclick="closeModal()"><i class="fas fa-xmark"></i></button>
    </div>
    <div style="text-align:center;font-size:2.8rem;margin-bottom:16px">✅</div>
    <p style="text-align:center;margin-bottom:16px;font-size:0.92rem;color:#3a5068">Purchase successful! Copy your link below:</p>
    <div class="copy-wrap">
    ...
    <div style="font-size:0.8rem;color:#6b8ba4;text-align:center"><i class="fas fa-shield-halved" style="color:#0a8c6a"></i> Re-purchase anytime to retrieve your link again.</div>
  </div>
</div>
  ...
  async function buyItem(type,id,btn){
    ...
    try{
      ...
      if(data.success){
        ...
        if(!data.already_owned){btn.innerHTML='<i class="fas fa-check"></i> Unlocked';btn.classList.add('owned');}
